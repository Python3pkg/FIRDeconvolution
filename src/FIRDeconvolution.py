...
class FIRDeconvolution(object): 
	...
	def __init__(self, signal, events, event_names = [], covariates = None, durations = None, sample_frequency = 1.0, deconvolution_interval = [-0.5, 5], deconvolution_frequency = None):
		...
		self.events = dict(list(zip(self.event_names, events)))
		...
		if covariates == None:
			self.covariates = dict(list(zip(self.event_names, [np.ones(len(ev)) for ev in events])))
		else:
			self.covariates = covariates

		if durations == None:
			self.durations = dict(list(zip(self.event_names, [np.ones(len(ev))/deconvolution_frequency for ev in events])))
		else:
		...
		self.event_times_indices = dict(list(zip(self.event_names, [((ev + self.deconvolution_interval[0])*self.deconvolution_frequency).astype(int) for ev in events])))
		# convert the durations to samples/ indices also
		self.duration_indices = dict(list(zip(self.event_names, [(self.durations[ev]*self.deconvolution_frequency).astype(int) for ev in self.event_names])))
